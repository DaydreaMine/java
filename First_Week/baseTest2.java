...
2.下列的哪个赋值语句是不正确的？
        ...
        B）double d = 5.3E12;
        C）float  d = 3.14f ;
        D）double  f=11.1E10f;//double f=11.1E10
...
A.666
        ...
        多选题
        8.下面哪种情况可以实现自动转换 (A,C)
        A）byte型转换成int型
        B）int型转换成byte型
        C）float型转换成double型                      
        D）double型转换成float型

        下面main()方法的定义哪些是正确的 (C A)
...
9如下哪些是Java中正确的整数表示（ A ）
        ...
        已知x=2, y=3, z=4，则经过z- = --y –x--运算后，z的值为（ A ）。
        （A）1 （B）2 （C）3 （D）4

        表达式(12==0) && (1/0 < 1)的值为（ D ）。
        ...
        11．Java语言中的逻辑与( && )和逻辑或( || )运算采用（ boolean ）方式进行运算。
        12．假设i=10, j=20, k=30，则表达式!(i<j+k) || !(i+10<=j) 的值为（ true ） 。

        若已初始化int型变量a，b和c，则当a是b和c的公约数时为true的逻辑表达式是( b%a=0||c%a=0 )。
        表达式(int)((double)(3)/2)的值是(     )。
        表达式5.3+(int)(8.5+4.6)/3%4的值是(    )。
        表达式19/3*3+19%3的值是(    )。
        浮点型数据根据数据存储长度和数值精度的不同，进一步分为 (  )和(  )两种类型。
        使用关键字(    )来定义逻辑变量。
        一个long型数据在内存占(   )个字节。
        表示制表符(水平空格)的字符常量是(   )。
        表示换行符的字符常量是(   )。
        ...
        已知double x=8.5, y=5.8;则表达式x++>y--值为(  )。
        执行语句inta, b, c; a=1; b=3; c=(a+b>3 ? ++a : b++);后，b的值为(   )。
        语句System.out.println(2/3*4.0);输出(  )。
        语句System.out.println(6.0*2/3);输出(  )。
        已知字符'1'的Unicode值为49，则语句System.out.println('1'+2);输出(  )。
        语句System.out.println(1+""+2);输出(  )。
        ...
        若int型变量a和b都是0，则执行语句boolean c = a == 0 || ++b == 0;语句后b的值为( 1 )。
        若int型变量a和b都是0，则执行语句boolean c = a != 0 && ++b == 0;语句后b的值为( 0 )
        已知doublex = 6.5, y = 6.5;则表达式x++>y--值为( true )。
        若char型变量c存储的是一个大写字母，则输出其对应小写字母的语句是( c+32 )。
        当char型变量c存储的是字母'c'的大小写时才为true的表达式是(  )。
        执行下列程序代码的输出结果是( 32  )。
        int a = 10 ;
        int i , j ;
        i = ++a ;
        ...
        编程题:
        1.编写Java程序，实现判断变量x是奇数还是偶数。//Task6
        2.X%2==0?偶数：奇数
